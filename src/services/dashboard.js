...
export const dashboardService = {
    ...
    async getDashboardData(filters = {}) {
        ...
        const url = `${API_BASE}/dashboard?${queryParams.toString()}`;
     ...
     */
    async getRouters() {
        const url = `${API_BASE}/dashboard?action=get_routers`;
        try {
    ...
    async getStaff() {
        try {
            const response = await authService.authenticatedFetch(`${API_BASE}/staff`);
    ...
    async updateStaff(id, formData) {
        try {
            const response = await authService.authenticatedFetch(`${API_BASE}/staff?id=${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(formData)
            });
