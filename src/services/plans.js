...
export const plansService = {
    ...
    async getPlans(routerId = 'default') {
        const url = `${API_BASE}/hotspot_plans?router_id=${routerId}&t=${new Date().getTime()}`;
        try {
    ...
    async savePlans(routerId, plans, changedPlan = null, action = 'add', deletedPlanName = null) {
        const url = `${API_BASE}/hotspot_plans`;
        try {
            const response = await authService.authenticatedFetch(url, {
                method: 'POST',
    ...
    async getSystemConfig(routerId = 'default') {
        const url = `${API_BASE}/system_config?router_id=${routerId}&t=${new Date().getTime()}`;
        try {
            const response = await authService.authenticatedFetch(url);
    ...
    async saveSystemConfig(routerId, configData) {
        const url = `${API_BASE}/system_config`;
        try {
            const response = await authService.authenticatedFetch(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
