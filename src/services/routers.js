...
export const routerService = {
    ...
    async getRouters({ page = 1, limit = 25 } = {}) {
        ...
        const url = `${API_BASE}/routers?${queryParams.toString()}`;
        try {
    ...
    // Get specific router
    async getRouter(id) {
        const url = `${API_BASE}/routers?id=${id}&t=${new Date().getTime()}`;
        try {
            const response = await authService.authenticatedFetch(url);
    ...
    async deleteRouter(id) {
        const url = `${API_BASE}/routers?id=${id}`;
        try {
            const response = await authService.authenticatedFetch(url, {
                method: 'DELETE',
            });
    ...
    async pingRouter(ip, port) {
        const url = `${API_BASE}/routers?action=ping`;
        try {
            const response = await authService.authenticatedFetch(url, {
                method: 'POST',
    ...
    async restartRouter(ip, port) {
        const url = `${API_BASE}/routers?action=restart`;
        try {
            const response = await authService.authenticatedFetch(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
