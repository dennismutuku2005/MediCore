    ...
    <div className="animate-in fade-in duration-500 space-y-8">
      <div className="bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-600 text-white rounded-xl p-10 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="text-[10px] text-blue-100 mb-3 opacity-70">Ward</div>
          ...
          <Badge status="completed" className="bg-white/20 text-white border-white/40 border-2 backdrop-blur-md px-4 py-2">Active</Badge>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <StatCard icon={<PatientsIcon size={20} />} label="Patients" value={stats.totalAssigned} trend="Patients under care" trendUp />
        <StatCard icon={<BedIcon size={20} />} label="Beds" value={stats.availableBeds} trend="Sector Capacity 24" trendUp={false} iconBg="#ecfdf5" iconColor="#059669" />
        <StatCard icon={<ClipboardIcon size={20} />} label="Tasks" value={stats.pendingTasks} trend="Action required" trendUp={false} iconBg="#fffbeb" iconColor="#d97706" />
        <StatCard icon={<PillIcon size={20} />} label="Meds due" value={stats.medsDue} trend="Current window" trendUp={false} iconBg="#fee2e2" iconColor="#dc2626" />
      </div>

      <div className="bg-white border border-slate-200 rounded-xl p-8 shadow-sm">
        <div className="mb-4 border-b border-slate-50 pb-4">
          <h3 className="text-sm font-medium text-slate-800">Shift summary</h3>
        </div>
        <div className="relative">
          <div className="absolute top-0 left-0 w-1 h-full bg-blue-600/20 rounded" />
          <p className="text-sm font-medium text-slate-600 leading-relaxed pl-6">
