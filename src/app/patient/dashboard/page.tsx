  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="bg-white border border-slate-200 rounded p-6 shadow-sm">
        <h1 className="text-xl font-black text-slate-900 tracking-tight">Salutations, {user?.name || 'Patient'} 👋</h1>
      ...
      <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden">
        ...
        <div className="p-5 space-y-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-black text-slate-800 uppercase">{data?.nextAppointment?.doctor || 'Unassigned'}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-bold text-slate-400 uppercase tracking-tighter">Clinical Department</span>
            <span className="font-bold text-slate-800">{data?.nextAppointment?.department || 'N/A'}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-bold text-slate-400 uppercase tracking-tighter">Reason for Encounter</span>
            <span className="font-bold text-slate-800">{data?.nextAppointment?.reason || 'Routine Checkup'}</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white border border-slate-200 rounded p-6 text-center shadow-sm">
          <div className="text-xl font-black text-slate-900">{data?.bloodPressure || '120/80'}</div>
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">Latest Pressure</div>
        </div>
        <div className="bg-white border border-slate-200 rounded p-6 text-center shadow-sm">
          <div className="text-xl font-black text-slate-900">{data?.weight || '0'} KG</div>
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">Current Mass</div>
        </div>
        <div className="bg-white border border-slate-200 rounded p-6 text-center shadow-sm">
          <div className="text-xl font-black text-slate-900">{data?.heartRate || '0'} BPM</div>
