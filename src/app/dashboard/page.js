...
function DashboardContent() {
    ...
    const canSeeIncome = isAdmin || policies.includes('view_income')
    ...
    const fetchRouters = async (pageValue = 1) => {
        setIsRoutersLoading(true);
        try {
            const res = await dashboardService.getRouterStatus({ page: pageValue, limit: 5 });
            if (res?.status === 'success') {
                const initialRouters = (res.data.router_status || []).map(r => ({ ...r, status: 'pinging' }));
                setRouters(initialRouters);
                setRouterPagination({ page: res.pagination.page, hasMore: res.pagination.has_more });

                // Individual pings to avoid blocking the main UI thread during load
                initialRouters.forEach(async (device) => {
                    if (!device.ip || device.ip === '0.0.0.0') return;
                    try {
                        const pingRes = await routerService.pingRouter(device.ip, device.winbox_port || 8728);
                        ...
                        setRouters(prev => prev.map(r => 
                            r.ip === device.ip ? { 
                                ...r, 
                                status: statusText,
                                load: isOnline ? (stats?.cpu || "Checked") : "0%",
                                uptime: isOnline ? (stats?.uptime || "Running") : "Disconnected"
                            } : r
                        ));
                    } catch (e) {
                        setRouters(prev => prev.map(r => 
                            r.ip === device.ip ? { ...r, status: 'Offline', load: '0%', uptime: 'Error' } : r
                        ));
                    }
                });
            }
    ...
    const metrics = widgets ? (canSeeIncome ? [
        ...
        { label: "Entries", value: (widgets.active_users.value || 0).toString(), note: 'Todays Entries', icon: Activity, color: 'text-green-500', bg: 'bg-green-500/10', href: '/dashboard/entries' },
        ...
        { label: "Online Customers", value: (widgets.online_customers?.value || 0).toString(), note: 'Live Now', icon: Wifi, color: 'text-emerald-500', bg: 'bg-emerald-500/10', href: '/dashboard/customers/active' },
        ...
        { label: "Active Stations", value: `${activeRouters}/${totalRouters}`, note: 'Healthy nodes', icon: Network, color: 'text-pace-purple', bg: 'bg-pace-purple/10', href: '/dashboard/routers' },
        { label: "Live Entries", value: (widgets.active_users.value || 0).toString(), note: 'Todays Success', icon: Smartphone, color: 'text-blue-500', bg: 'bg-blue-500/10', href: '/dashboard/entries' },
        { label: "Online Now", value: (widgets.online_customers?.value || 0).toString(), note: 'Active Devices', icon: Wifi, color: 'text-green-500', bg: 'bg-green-500/10', href: '/dashboard/customers/active' },
        { label: "User Growth", value: (widgets.customers_month?.value || 0).toString(), note: 'Unique MACs', icon: Users, color: 'text-admin-dim', bg: 'bg-pace-bg-subtle' },
        { label: "Network Health", value: widgets.system_health.value || '98%', note: 'System Integrity', icon: Activity, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
        { label: "Latency Status", value: "Normal", note: 'Optimized response', icon: RefreshCw, color: 'text-orange-500', bg: 'bg-orange-500/10' },
    ]) : [];
    ...
    return (
        <div className="space-y-6 font-figtree animate-in fade-in duration-700 max-w-[1600px] mx-auto pb-10 sm:px-0">
                    ...
                    return (
                        <CardWrapper
                            ...
                            className={cn(
                                ...
                                (metric.href || metric.label === "Month Revenue") && "cursor-pointer"
                            )}
                        >
                ...
                <div className="lg:col-span-8 bg-card-bg border border-pace-border rounded-xl p-3 sm:p-6 shadow-sm min-w-0">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
                        ...
                        <div className="flex gap-4">
                            {canSeeIncome && <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-pace-purple" /><span className="text-[9px] font-medium text-gray-400 uppercase tracking-wider">Revenue</span></div>}
                            <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-green-500" /><span className="text-[9px] font-medium text-gray-400 uppercase tracking-wider">{canSeeIncome ? 'Activity' : 'System Utilization'}</span></div>
                        </div>
                    </div>
                        ...
                        {isChartsLoading ? <Skeleton className="w-full h-full rounded-lg" /> : (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={charts.map(c => ({ label: c.day, amount: c.amount, entries: c.entries }))} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                    ...
                                    {canSeeIncome && <Area type="monotone" dataKey="amount" stroke="#7c3aed" strokeWidth={3} fillOpacity={1} fill="url(#colorEarnings)" animationDuration={1000} />}
                                    ...
                                    <div key={tx.id} className="flex items-center justify-between py-3 border-b border-pace-border last:border-0 hover:bg-pace-bg-subtle -mx-1 px-1 rounded-lg transition-colors group">
                                        ...
                                        <div className="text-right">
                                            {canSeeIncome ? (
                                                ...
                                                <>
                                                    <div className="px-2 py-0.5 rounded-full bg-pace-bg-subtle text-admin-dim border border-pace-border text-[8px] font-bold uppercase">SECURE</div>
                                                    <p className="text-[9px] text-gray-400 mt-1 uppercase font-medium">{tx.id}</p>
                                                </>
                                            )}
                                        </div>
                                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                <div key={idx} className="p-2.5 border border-pace-border rounded-xl hover:bg-pace-bg-subtle transition-all group flex justify-between items-center">
                                    ...
                                    {device.status === 'pinging' ? (
                                        ...
                                        <Badge 
                                            ...
                                            className="text-[8px] font-medium uppercase tracking-wider px-2 py-0.5 border-none"
                                        >
                                            {device.status}
                                        </Badge>
                                    )}
                ...
                {/* Revenue Breakdown / Activity Detail */}
                <div className="lg:col-span-8 bg-card-bg border border-pace-border rounded-xl p-3 sm:p-6 shadow-sm order-1 lg:order-2">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-2">
                        <div>
                            <h4 className="text-base font-bold text-pace-purple uppercase">{canSeeIncome ? 'Revenue Trend' : 'Activity Distribution'}</h4>
                            <p className="text-[10px] text-gray-400 font-semibold tracking-wider uppercase">{canSeeIncome ? 'Earnings Growth' : 'Entry Frequency'}</p>
                        </div>
                        {canSeeIncome && <div className="px-3 py-1 bg-green-500/10 text-green-500 text-[9px] font-bold rounded-full uppercase tracking-widest border border-green-500/20">+12.5% vs Prev</div>}
                    </div>
                        ...
                        {isChartsLoading ? <Skeleton className="w-full h-full rounded-lg" /> : (
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={charts} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                    ...
                                    <Bar dataKey={canSeeIncome ? "amount" : "entries"} radius={[4, 4, 4, 4]} barSize={32} fill={canSeeIncome ? "#7c3aed" : "#22c55e"}>
                                        {charts.map((entry, index) => (
                                            <Cell key={`cell-${index}`} fill={index === charts.length - 1 ? (canSeeIncome ? '#7c3aed' : '#22c55e') : '#e5e7eb'} />
                                        ))}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
