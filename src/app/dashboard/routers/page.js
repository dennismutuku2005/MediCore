...
import { Search, Plus, Wifi, Terminal, CreditCard, Activity, CheckCircle2, XCircle, MoreHorizontal, Trash2, Edit, Signal, Power, AlertTriangle, Loader2, ChevronRight, Store, Shield } from 'lucide-react'
...
function RoutersContent() {
                ...
                fetchedRouters.forEach(async (router) => {
                    try {
                        ...
                        const isOnline = 
                            ...
                            (stats?.success === true) ||
                            (typeof stats?.cpu === 'string' && stats.cpu.includes('%')) ||
    ...
    // Helper function to get display text for account type
    const getAccountTypeDisplay = (type) => {
        if (!type) return 'Not Set';
        if (type.toLowerCase() === 'till') return 'Till Number';
        if (!isNaN(type)) return `Paybill ${type}`;
        return type.toUpperCase() + ' Paybill';
    }
                    ...
                    <table className="w-full text-left text-[11px] whitespace-nowrap">
                        <thead className="bg-card-bg border-b border-pace-border">
                            <tr className="text-admin-dim font-bold uppercase tracking-wider text-[10px]">
                                ...
                                <th className="px-4 py-3">Payment Config</th>
                                ...
                                filteredRouters.map((router) => (
                                    <tr key={router.id} className="hover:bg-pace-bg-subtle transition-colors group">
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            ) : router.status === 'inactive' || router.status === 'offline' ? (
                                                <div className="flex items-center gap-2 text-red-400/60 italic">
                                                    <XCircle size={12} />
                                        ...
                                        <td className="px-4 py-3">
                                            {user?.policies?.includes('income_policy') ? (
                                                <div className="flex items-center gap-2 text-admin-dim italic">
                                                    <Shield size={12} />
                                                ...
                                                <div className="space-y-1">
                                                    <Badge 
                                                        variant={router.accountType?.toLowerCase() === 'till' ? 'warning' : 'info'}
                                                        className="text-[9px] px-2 py-0.5 font-medium uppercase inline-flex items-center gap-1"
                                                    >
                                                        {router.accountType?.toLowerCase() === 'till' ? <Store size={10} /> : <CreditCard size={10} />}
                                                        {getAccountTypeDisplay(router.accountType)}
                                                    </Badge>
                                                    <p className="text-[10px] text-admin-dim font-mono">
                                                        {router.accountNumber || 'No reference set'}
                                                    </p>
                                                </div>
                                            )}
                                        </td>
            ...
            >
                {selectedRouter && (
                    <div className="space-y-3">
                        ...
                        {!user?.policies?.includes('income_policy') && (
                            <div className="bg-pace-bg-subtle p-3 rounded-2xl border border-pace-border">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-[9px] text-admin-dim font-bold uppercase tracking-wider">Payment Method</span>
                                    <Badge 
                                        variant={selectedRouter.accountType?.toLowerCase() === 'till' ? 'warning' : 'info'}
                                        className="text-[8px]"
                                    >
                                        {selectedRouter.accountType?.toLowerCase() === 'till' ? 'Buy Goods (Till)' : 'Paybill'}
                                    </Badge>
                                </div>
                                <div className="flex items-center gap-2 text-[11px]">
                                    {selectedRouter.accountType?.toLowerCase() === 'till' ? (
                                        <Store size={14} className="text-amber-500" />
                                    ) : (
                                        <CreditCard size={14} className="text-pace-purple" />
                                    )}
                                    <span className="font-medium text-pace-purple">{selectedRouter.accountNumber || 'Not set'}</span>
                                </div>
                            </div>
                        )}
            ...
            >
                <div className="space-y-5 py-2">
                    ...
                    <div className="space-y-4">
                        {/* Account Type Selection - Now includes TILL */}
                            ...
                            <div className="grid grid-cols-5 gap-2">
                                ...
                                {['kcb', 'equity', 'ncba'].map((type) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-[8px] text-admin-dim font-bold">#{BANK_PAYBILLS[type]}</span>
                                    </button>
                                ))}
                                
                                {/* Till Option */}
                                <button
                                    onClick={() => setEditData({ ...editData, accountType: 'till' })}
                                    className={cn(
                                        "flex flex-col items-center justify-center p-2 rounded-2xl border transition-all gap-1",
                                        editData.accountType?.toLowerCase() === 'till'
                                            ? "bg-amber-500/10 border-amber-500 border-2"
                                            : "bg-pace-bg-subtle border-pace-border hover:border-amber-500/40"
                                    )}
                                >
                                    <Store size={14} className={editData.accountType?.toLowerCase() === 'till' ? "text-amber-500" : "text-admin-dim"} />
                                    <span className={cn(
                                        "text-[10px] font-black uppercase tracking-tighter",
                                        editData.accountType?.toLowerCase() === 'till' ? "text-amber-500" : "text-admin-dim"
                                    )}>
                                        Till
                                    </span>
                                </button>

                                {/* Custom Paybill Option */}
                                <button
                                    onClick={() => setEditData({ ...editData, accountType: isNaN(editData.accountType) ? '' : editData.accountType })}
                                    className={cn(
                                        "flex flex-col items-center justify-center p-2 rounded-2xl border transition-all gap-1",
                                        (!isNaN(editData.accountType) && editData.accountType !== '' && editData.accountType !== null)
                                            ? "bg-emerald-500/10 border-emerald-500 border-2"
                                            : "bg-pace-bg-subtle border-pace-border hover:border-emerald-500/40"
                                    )}
                                >
                                    <CreditCard size={14} className={(!isNaN(editData.accountType) && editData.accountType !== '' && editData.accountType !== null) ? "text-emerald-500" : "text-admin-dim"} />
                                    <span className={cn(
                                        "text-[10px] font-black uppercase tracking-tighter",
                                        (!isNaN(editData.accountType) && editData.accountType !== '' && editData.accountType !== null) ? "text-emerald-500" : "text-admin-dim"
                                    )}>
                                        Custom
                                    </span>
                                </button>
                        ...
                        {/* Custom Paybill Input (Only if accountType is being edited to be numeric) */}
                        {(!editData.accountType || !['kcb', 'equity', 'ncba', 'till'].includes(editData.accountType.toLowerCase())) && (
                            <div className="space-y-2 animate-in fade-in slide-in-from-top-1 duration-300">
                                ...
                                <input
                                    type="text"
                                    value={!['kcb', 'equity', 'ncba', 'till'].includes(editData.accountType?.toLowerCase()) ? editData.accountType : ''}
                                    onChange={(e) => setEditData({ ...editData, accountType: e.target.value })}
                                    placeholder="Enter Paybill (e.g. 4183811)"
                                    className="w-full bg-pace-bg-subtle border border-emerald-200 rounded-2xl px-4 py-3 text-sm font-bold text-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all placeholder:text-admin-dim"
                                />
                            </div>
                        )}

                        {/* Account Number / Till Number Input */}
                        <div className="space-y-2">
                            <label className="text-[10px] text-gray-400 font-bold uppercase tracking-widest ml-1 flex items-center gap-2">
                                {editData.accountType?.toLowerCase() === 'till' ? (
                                    <>
                            ...
                            <div className="relative group">
                                <input
                                    ...
                                    placeholder={editData.accountType?.toLowerCase() === 'till' 
                                        ? "e.g. 522533" 
                                        : "e.g. PACE_001"}
                                    className={cn(
                                        "w-full bg-pace-bg-subtle border rounded-2xl px-4 py-3 text-sm font-bold focus:outline-none focus:ring-4 transition-all placeholder:text-admin-dim",
                                        editData.accountType?.toLowerCase() === 'till'
                                            ? "text-amber-600 focus:ring-amber-500/5 focus:border-amber-500 border-amber-200"
                                            : "text-pace-purple focus:ring-pace-purple/5 focus:border-pace-purple border-pace-border"
                                    )}
                                />
                                {editData.accountType?.toLowerCase() === 'till' && (
                                    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1.5 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-200">
                                        <Store size={10} className="text-amber-500" />
                                        <span className="text-[8px] font-black text-amber-600 uppercase tracking-widest">Till</span>
                                    </div>
                                )}
                            </div>
                            <p className="text-[9px] text-gray-400 font-medium pl-1 italic">
                                {editData.accountType?.toLowerCase() === 'till' 
                                    ? '* This till number will be used for Buy Goods (CustomerBuyGoodsOnline) transactions'
                                    : '* This reference will be shown to customers during STK push as the account number'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <AlertTriangle size={16} className={editData.accountType?.toLowerCase() === 'till' ? "text-amber-500" : "text-amber-500"} />
                        <p className={cn(
                            "text-[10px] leading-normal font-medium uppercase tracking-tight",
                            editData.accountType?.toLowerCase() === 'till' ? "text-amber-600" : "text-amber-500"
                        )}>
                            {editData.accountType?.toLowerCase() === 'till' 
                                ? 'Till numbers are used for Buy Goods transactions. Ensure this is a valid M-Pesa Till number.'
