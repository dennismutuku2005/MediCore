...
import { appointments as initialAppts } from '@/lib/mockData';
import { CalendarIcon, ClockIcon, EditIcon, CheckCircleIcon, CloseIcon } from '@/components/ui/Icons';

export default function DoctorAppointments() {
  const [loading, setLoading] = useState(true);
  const [list, setList] = useState(initialAppts);
  ...
  const myAppts = list.filter(a => a.doctor === 'Dr. Amina Odhiambo');

  const updateStatus = (id: string, status: string) => {
    setUpdating(true);
    setTimeout(() => {
      setList(prev => prev.map(a => a.id === id ? { ...a, status: status as any } : a));
      setUpdating(false);
      setSelected(null);
    }, 800);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white">
          ...
          <Badge status="active">LIVE QUEUE</Badge>
        </div>
            ...
            <tr key={a.id} className="hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 group select-none cursor-pointer" onClick={() => setSelected(a)}>
              <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-slate-100 text-slate-400 flex items-center justify-center font-bold text-xs uppercase">{a.patient.charAt(0)}</div>
                  <span className="text-sm font-black text-slate-800 tracking-tight">{a.patient}</span>
                </div>
              </td>
              <td className="px-6 py-4 text-sm text-slate-500 font-medium">{a.date}</td>
              <td className="px-6 py-4">
                <div className="flex items-center gap-1.5 text-blue-600">
                  <ClockIcon size={14} />
                  <span className="text-sm font-black italic">{a.time}</span>
                </div>
              </td>
          ...
          <div className="space-y-8 py-2">
            <div className="flex flex-col items-center pb-6 border-b border-slate-50">
              <div className="w-16 h-16 rounded bg-slate-100 flex items-center justify-center text-slate-400 font-black text-2xl mb-4 shadow-inner">
                {selected.patient.charAt(0)}
              </div>
              <h4 className="text-lg font-black text-slate-900 tracking-tight">{selected.patient}</h4>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Scheduled Encounter: {selected.date} @ {selected.time}</p>
            </div>
